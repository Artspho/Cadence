...
describe("getPlafondAreAt", () => {
  it("FCT antérieure au 01/01/2026 : plafond de 174,80 €, pas la valeur courante", () => {
    expect(getPlafondAreAt("2025-12-31", franceTravailConfig)).toBe(PLAFOND_2024);
...
describe("calculerAJBrute — le clamp haut suit la date d'ouverture du droit", () => {
  ...
  it("à la date pivot exacte : 181,18 €, et 174,80 € la veille", () => {
    expect(calculerAJBrute({ ...params, dateEffet: PIVOT }).brut).toBe(PLAFOND_2026);
    expect(calculerAJBrute({ ...params, dateEffet: "2025-12-31" }).brut).toBe(PLAFOND_2024);
  });
...
describe("calculerAJBrutePourFenetre — dérive la date de la fenêtre (dateFin = FCT retenue)", () => {
  ...
  it("fenêtre se terminant en 2025 : plafond 2024 ; en 2026 : plafond 2026", () => {
    expect(calculerAJBrutePourFenetre(fenetreSeTerminantLe("2025-06-01"), 700, 400_000, 700, franceTravailConfig).brut).toBe(PLAFOND_2024);
...
describe("renouvellement anticipé simulé sur une FCT passée — régression du 03/08/2026", () => {
  ...
  it("FCT au 01/06/2025 : le plafond appliqué est celui de 2025 (174,80 €), pas 181,18 €", () => {
    ...
    expect(r.nouveau.ajBrute.brut).toBe(PLAFOND_2024);
