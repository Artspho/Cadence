...
- **Profils mixtes (A10 + A8 + RG) : détection par auto-déclaration, plus UNE contradiction interne — le cas principal reste indétectable.** Le garde-fou (`lib/profilHorsPerimetre.ts`, §11.A) couvre deux familles de cas, avec deux traitements distincts :
  1. **Déclaration explicite** (`regimeDeclare` = « mixte » ou « inconnu »/je-ne-sais-pas, qui suivent le même chemin conservateur) → **bloquant** : aucun statut ni montant n'est affiché, l'utilisateur est renvoyé vers France Travail.
  2. **Contradiction interne** : le profil se déclare A10 pur alors que `salairesHorsAnnexe10PRA > 0` (champ dont le libellé même désigne du technicien A8 / régime général). Les deux saisies ne peuvent pas être vraies ensemble, mais on ne sait pas laquelle est fausse → **non bloquant** : alerte `critique` (`salaires_hors_a10_contradictoires`), montants ARE masqués sur tous les écrans qui en affichent, app pleinement utilisable le temps que l'utilisateur tranche dans « Mon profil ». Déclenchement strict sur `> 0` : ni `null`, ni `0`, ni « champ renseigné » ne suffisent, pour ne pas alerter qui saisit honnêtement zéro. Le champ `salairesHorsAnnexe10PRA` est saisissable **quelle que soit la situation du profil** (première admission comme réadmission), dans la carte du régime déclaré de « Mon profil » — à côté de l'autre moitié de la contradiction, donc corrigeable au même endroit. Il vivait jusqu'au 2026-07-28 dans la section « Mon indemnisation en cours », réservée à la réadmission : une première admission ne pouvait alors créer la contradiction que par import JSON manuel, et n'était donc jamais avertie depuis l'UI. Ni le moteur de franchise salaires ni le garde-fou ne regardent `profil.situation` : cette restriction était un effet de bord de l'emplacement du champ, pas une règle métier. Le fait lui-même n'est plus rédigé qu'une fois (`content/contradictionHorsA10.ts`, partagé par l'alerte et le bandeau), et le centre d'alertes masque son alerte là où le bandeau dédié est déjà affiché (`lib/alertesAffichage.ts`) — sans jamais afficher « Aucune alerte » à côté d'un bandeau critique.
  
  **Ce qui reste hors d'atteinte, et pourquoi ce n'est pas un oubli :** le cas principal — un technicien intermittent qui répond « je relève uniquement de l'Annexe 10 » et saisit ses vacations comme des cachets d'artiste — demeure **structurellement indétectable**. `TypeContrat` ne connaît que `artiste | enseignement | formation | ptp` : il n'existe aucun champ pour représenter une vacation A8 ou un emploi au régime général, donc rien à détecter dans les contrats. Même limite pour un CDI hors spectacle en parallèle. Aucune heuristique n'est acceptable ici : deviner à partir de `Contrat.employeur` (texte libre) manquerait les vrais cas tout en en inventant d'autres, et `Contrat.type === "enseignement"` ou `territoire === "eee_suisse_uk"` seraient de purs faux positifs (l'enseignement est modélisé À L'INTÉRIEUR des règles A10 ; un cachet joué à l'étranger et déclaré en France reste couvert, cf. la limite U1/PDU1 ci-dessus). Fermer ce trou suppose d'ajouter au modèle un type de contrat hors A10 — c'est-à-dire **ouvrir la modélisation Annexe 8 / article 65, renvoyée au §11.C**, pas étendre un garde-fou. Tant que ce n'est pas fait, un profil mixte qui répond « non » à tort reste un faux « sans le savoir » : c'est le risque résiduel assumé de la bêta.
...
- **Préremplissage automatique du revenu par IA — « Premium »** : deux fonctionnalités, réservées ensemble aux utilisateurs payants. Sans analyse IA, seule la **saisie manuelle** reste possible — celle qui existe déjà aujourd'hui, et qui reste gratuite. Le préremplissage propose, il ne se substitue jamais à la saisie. Les deux dépendent de l'**import PDF généralisé** ci-dessus. Statut : périmètre balisé, rien construit, rien spécifié en détail.
  1. **« Allocations ARE »** — lecture automatique des documents France Travail déjà déposés (relevés de situation, notification d'ouverture de droits, déclaration fiscale annuelle) pour en extraire le montant exact.
  2. **« Salaire net imposable »** — lecture automatique des contrats / bulletins de paie / AEM déjà déposés pour en extraire le montant exact.

  🚫 **Prérequis bloquant, à traiter AVANT ces deux fonctionnalités** : Cadence n'a aujourd'hui **ni comptes ni paiement** — c'est une SPA 100 % locale, et backend/comptes sont explicitement hors bêta (cf. le premier item de ce §11.B). Un simple drapeau « premium » dans le code ne serait pas une restriction réelle : tout est lisible et modifiable côté client. Il faut donc poser une brique **comptes + paiement** avant de pouvoir réserver quoi que ce soit à des utilisateurs payants.

  ⚠️ Prérequis technique à revalider au moment de construire (non tranché à ce jour) : la reconstitution du net imposable depuis un document de paie. Trois cas, dans cet ordre :
  1. Le document affiche une ligne « net imposable » séparée → la lire directement, le prélèvement à la source (PAS) n'entre pas en jeu.
  2. Sinon → reconstituer : `net imposable = net à payer + retenue à la source prélevée`.
  3. **Piège identifié** : sur les périodes où le taux de PAS est à 0 %, la ligne « retenue à la source » peut ne pas figurer du tout sur le document (rien à prélever, rien à afficher). L'IA ne doit PAS traiter cette absence comme un échec d'extraction : c'est un cas normal où `retenue = 0` et `net imposable = net à payer` tel quel. Traiter ce cas comme une erreur ferait échouer l'import de bulletins parfaitement valides ; l'assimiler à un « champ manquant » à re-demander à l'utilisateur serait aussi trompeur.

  ⚠️ Second piège, propre à R : le net imposable d'un bulletin **n'est pas** R. R additionne quatre composantes puis les plafonne (cf. `docs/spec_frais_reels_cadence.md` §3), et pour un profil `artiste_enseignant_accessoire` les forfaits 14 %/5 % ne portent que sur la part **artistique** : sommer tous les bulletins déposés produirait un R faux pour tout profil mixte. À trancher : comment séparer artistique et enseignement à partir d'un bulletin.
- **Monétisation envisagée : 2 mois d'essai gratuit, puis app payante dans son ensemble** (pas de fonctionnalités séparées gratuit/premium — décision du 01/08/2026). Statut : **intention actée, architecture non tranchée.** Questions à trancher avant tout code, dans cet ordre :
  1. Que deviennent les données locales d'un utilisateur qui ne paie pas après l'essai ? Deux options aux implications très différentes :
     a) L'app se bloque mais les données restent intactes en local (cohérent avec le principe local-first actuel, mais modèle UX à valider : « tes données sont là, mais inaccessibles sans payer »).
     b) L'app nécessite un backend actif pour fonctionner, pour tout le monde — abandonne de fait le local-first, changement d'architecture bien plus lourd qu'un simple ajout de compte.
  2. Un minimum de compte est incontournable dans les deux cas, car la temporalité de l'essai (2 mois) doit être fiable et non réinitialisable en vidant le cache navigateur — mais ce compte peut rester très léger (email + date de début d'essai + statut payé, pas nécessairement un vrai profil utilisateur complet).
  3. Paiement : Stripe est le standard du secteur pour ce genre d'essai gratuit → abonnement, gère nativement les relances et la carte. Frais Stripe : ~1,5 % + 0,25 €/transaction (pas de fixe mensuel).
  4. Coût d'hébergement estimé si compte+paiement passent par un backend (documents restant en local, hors de ce backend) : ~20 $/mois Vercel Pro (obligatoire commercialement, palier gratuit interdit à l'usage commercial) + Supabase Pro ~25 $/mois recommandé plutôt que le palier gratuit (celui-ci se met en pause après 7 j d'inactivité — risque inacceptable une fois de vrais paiements en jeu) → **~45 $/mois de frais fixes** au total, hors commission Stripe.

  **Décision explicite prise le 01/08/2026 : ne pas trancher maintenant.** À reprendre une fois que la bêta entre amis aura donné un premier signal d'usage (est-ce que les gens reviennent, est-ce qu'ils feraient confiance à l'app) — trancher l'architecture de monétisation avant d'avoir ce signal reviendrait à optimiser un problème qu'on n'a pas encore confirmé avoir.
...
- ✅ **Comparaison ancien/nouveau droit pour un renouvellement anticipé** (31/07/2026) : prérequis bloquant levé — règle sourcée et validée (simulateur officiel simucalcul.pole-emploi-services.fr + cas réel à deux notifications), cf. `docs/validation.md` (cas Réel #1 = Notification 2, + B1/B2/B3/E1). Moteur : `engine/renouvellementAnticipe.ts` (`calculerRenouvellementAnticipe`), réutilise tel quel `periodeReference.ts`/`salaireReference.ts`/`areBrute.ts`/`areNette.ts`. Écran : section « Renouvellement anticipé » dans `MonProfil.tsx` (`components/RenouvellementAnticipe.tsx`), simulation seulement — pas encore la demande elle-même (item suivant). `tropPercuRisque` reste un booléen de prudence, jamais chiffré (`tropPercuChiffrable: false`) : aucun cas réel n'a encore confirmé de formule de trop-perçu exacte. Deux bugs pré-existants découverts et corrigés au passage (hors périmètre de ce chantier mais révélés par lui) : la fenêtre de référence n'était pas bornée par la FCT du droit précédent dans `periodeReference.ts` quand le seuil était déjà atteint sans extension ; le SJM (base de la retraite complémentaire/CSG-CRDS) était calculé sur le SR brut au lieu du SAR aménagé partout où un SAR s'applique (`App.tsx`, `Simulateur.tsx`, `cycles.ts`, `RevenusMensuels.tsx`). ⚠️ Écart non résolu, hors périmètre : à un SR extrême et irréaliste (~400 000 €), la formule A+B+C de `areBrute.ts` diverge du simulateur officiel (188,72 € contre 155,77 €) — jamais rencontré à un SR réaliste (Fictif #3 concorde à 50 000 €), à creuser séparément si un cas réel le confirme un jour. Rouvert le 01/08/2026 suite à un signalement d'un SR ~400 000 € affiché par l'app elle-même (pas une valeur de test volontaire) : investigation menée sur les données réelles disponibles (export du 31/07, 56 contrats, calcul reproduit avec le vrai moteur), **aucune reproduction obtenue** — SR calculé 6 049 €, somme de tous les contrats de toute la carrière 25 593 €, aucun outlier. **Élucidé le même jour : Benoît a vérifié de son côté et confirmé une erreur de saisie de sa part — CE N'ÉTAIT PAS un bug Cadence.** Chantier fermé sans aucune correction de code (cf. `docs/reprise.md`) ; l'écart de formule à un SR extrême décrit dans la phrase précédente reste, lui, une question distincte et toujours ouverte.
- **Interface de demande de renouvellement anticipé** : l'écran/le flux qui permet à l'utilisateur de faire concrètement la demande une fois informé qu'un réexamen anticipé est possible (formulaire, rappel des pièces à joindre, lien ou modèle de courrier vers France Travail selon les modalités réelles). L'item précédent est désormais levé (comparaison sourcée et validée) — ce chantier reste à faire : aujourd'hui l'écran renvoie seulement vers l'espace personnel France Travail (`RenouvellementAnticipe.tsx`), sans formulaire ni checklist dédiés.
