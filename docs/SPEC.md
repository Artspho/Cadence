...
- **Interface de demande de renouvellement anticipé** : l'écran/le flux qui permet à l'utilisateur de faire concrètement la demande une fois informé qu'un réexamen anticipé est possible (formulaire, rappel des pièces à joindre, lien ou modèle de courrier vers France Travail selon les modalités réelles). Dépend directement de l'item précédent : ne pas construire cette interface avant que la règle de comparaison ancien/nouveau droit soit sourcée et validée, sinon l'interface inciterait à une démarche dont on ne sait pas encore si elle est favorable à l'utilisateur — même prérequis bloquant, à ne pas contourner en traitant l'UI et le calcul comme deux chantiers indépendants.
