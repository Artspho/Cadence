...
- "profil_infos" : dateAnniversaire, dateNaissance, dateAnniversairePrecedente, situation
  (premiere_admission/readmission), dureeDroitsMois (12 ou 6).
- "periode_assimilee" : maternité, adoption, accident du travail, suspension de contrat -> type,
  ...
  dates et un avertissement demandant à l'utilisateur de choisir manuellement.
...
- "info_seule" : toute donnée utile pour vérifier ou recaler les calculs de l'app qui n'a pas de
  destination sûre (SR, NHT, jours non indemnisés, taux d'imposition, périodes assimilées
  ambiguës, etc.) — ne la perds jamais, range-la en "info_seule" plutôt que d'inventer.

Champs JAMAIS à proposer : "regimeDeclare" (signalé par l'utilisateur uniquement),
"salairesHorsAnnexe10PRA" (crée une contradiction si proposé seul), toute constante
réglementaire (plafonds enseignement/formation), "activiteHorsAnnexe10" (déprécié), et le point
de départ d'affichage (aucun document ne le contient).

Règles impératives :
- Jamais de valeur inventée : champ illisible, absent ou ambigu -> pas de proposition, plutôt un
  message dans "avertissementsGeneraux" ou une proposition "info_seule".
...
const RESPONSE_SCHEMA = {
  ...
  properties: {
    typeDocumentDetecte: {
      type: "string",
      enum: ["bulletin_paie", "aem", "notification_admission", "releve_situation", "declaration_fiscale_annuelle", "attestation_cpam", "non_reconnu"],
    },
    propositions: {
      type: "array",
      items: {
        anyOf: [
            ...
            properties: {
              ...
              donnees: {
                ...
                properties: {
                  ...
                  dateLimiteIndemnisation: { type: "string" },
                  tauxPrelevementSource: { type: "number" },
                },
              },
              confiance: { type: "object" },
              justification: { type: "string" },
          ...
          {
            type: "object",
            title: "PropositionProfilInfos",
            properties: {
              cible: { type: "string", enum: ["profil_infos"] },
              donnees: {
                type: "object",
                properties: {
                  dateAnniversaire: { type: "string" },
                  dateNaissance: { type: "string" },
                  dateAnniversairePrecedente: { type: "string" },
                  situation: { type: "string", enum: ["premiere_admission", "readmission"] },
                  dureeDroitsMois: { type: "number", enum: [12, 6] },
                },
              },
              confiance: { type: "object" },
              justification: { type: "string" },
          ...
          {
            type: "object",
            title: "PropositionPeriodeAssimilee",
            properties: {
              cible: { type: "string", enum: ["periode_assimilee"] },
              donnees: {
                type: "object",
                properties: {
                  type: { type: "string", enum: ["maternite", "adoption", "accident_travail", "ald", "suspension_contrat", "maladie_intercontrat"] },
                  dateDebut: { type: "string" },
                  dateFin: { type: "string" },
                },
              },
